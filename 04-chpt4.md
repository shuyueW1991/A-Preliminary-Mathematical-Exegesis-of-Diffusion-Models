...
Chapter 3 also provides mathematical tools to push forward the realization of the thinking in this chapter.
...
That they push the initial distribution towards converging gaussian is validated in Chapter 3.
...
Since \\(W_t - W_{t-1} \sim \mathcal{N}(0, \Delta t = 1)\\), let \\(z_{t-1} = W_t - W_{t-1} \sim \mathcal{N}(0, I)\\):
...
However, we only have the expression of \\(x_t\\) on \\(x_{t-1}\\):
...
So we accumulate that till \\(t=0\\), then we have 
...
$$\tilde{x} = x + \sigma \epsilon$$
 ...
 where \\(\epsilon \sim \mathcal{N}(0, I)\\) in Chapter 3,  according to 
...
However, this analogy is not convincing after all.
